...
/*
Added support for pulse dialing (11xx) and hangup reminder with ROH (receiver off-hook) tone.
...
static int analog_gendigittimeout = 10000;
static int analog_firstdigittimeout = 30000;
...
//Armen
//Making changes for rotary dialing to work with 11xx codes
	if ( (extlen < strlen(pickup_ext) && !strncmp(pickup_ext, exten, extlen)) || ((extlen < strlen(pickup_ext) && !strncmp(pickup_ext, exten, extlen)) && exten[1] != '1')) {
		ast_verb(3, "Returning 1 a \n");
		return 1;
	}
	...
	if ( (exten[0] == '*' && extlen < 3) || (exten[0] == '1' && extlen < 4) ) {
		if (extlen == 1) {
			ast_verb(3, "Returning 1 b \n");
			return 1;
		}
	...
	case ANALOG_SIG_FXOKS:
		...
		while (len < AST_MAX_EXTENSION-1) {
			...
			if (ast_exists_extension(chan, ast_channel_context(chan), exten, 1, p->cid_num) && !ast_parking_ext_valid(exten, chan, ast_channel_context(chan))) {
				...
				//Armen
				//Provide for a recording which reminds you to hang up and then plays the ROH tone.
				while (!res && p->fxsoffhookstate == 1) {
					if ((res = ast_streamfile(chan, "/var/lib/asterisk/sounds/hangup_reminder", ast_channel_language(chan)))) {
			...
			else if (!strcmp(exten, "*70") || !strcmp(exten, "1170"))  {
			...
			else if ((!strcmp(exten, "*67") || !strcmp(exten, "1167"))) {
				...
				else if (p->callreturn && (!strcmp(exten, "*69") || !strcmp(exten, "1169")))  {
				res = 0;
				...
				else if (!strcmp(exten, "*78") || !strcmp(exten, "1178")) {
				...
				else if ( !strcmp(exten, "*79") || !strcmp(exten, "1179") ) {
				...
				else if (p->cancallforward && ( !strcmp(exten, "*72") || !strcmp(exten, "1172") )) {
				res = analog_play_tone(p, idx, ANALOG_TONE_DIALRECALL);
				getforward = 1;
				memset(exten, 0, sizeof(exten));
				len = 0;
				}
				else if (p->cancallforward && (!strcmp(exten, "*73") || !strcmp(exten, "1173"))) {
			...
			else if ((!ast_strlen_zero(p->lastcid_num)) && (!strcmp(exten, "*60") || !strcmp(exten, "1160"))) {
			...
			else if ( !strcmp(exten, "*82") || !strcmp(exten, "1182") ) {
			...
			//Need to work on this to match 11xx pulse dial codes.
			else if (!ast_canmatch_extension(chan, ast_channel_context(chan), exten, 1,ast_channel_caller(chan)->id.number.valid ? ast_channel_caller(chan)->id.number.str : NULL) && !analog_canmatch_featurecode(exten)) {
				ast_verb(3, "Can't match %s from '%s' in context %s\n", exten,
